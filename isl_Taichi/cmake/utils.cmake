...
function(__find_pkg_then_add_target pkg_name pkg_exe lib_path)
    ...
    foreach(_LIB_NAME ${ARGN})
        #默认情况下，_LIB_TYPE 被设置为共享库（SHARED）。_LIB_SEARCH_NAME 是库的名称，在之后会被用于 find_library
        set(_LIB_SEARCH_NAME ${_LIB_NAME})
        set(_LIB_TYPE SHARED)
        #如果需要查找静态库，_LIB_SEARCH_NAME 被修改为静态库的名称格式，通常是添加库的前缀和后缀（例如 lib 和 .a
        ...
        #find_library 函数会在指定的路径 ${${pkg_name}_BASE_DIR}/${lib_path} 中查找库文件（例如 .a 或 .so 文件），并将结果赋值给 ${_LIB_NAME}_LIB。
        find_library(${_LIB_NAME}_LIB ${_LIB_SEARCH_NAME} PATHS ${${pkg_name}_BASE_DIR}/${lib_path} NO_DEFAULT_PATH)

        if(NOT ${_LIB_NAME}_LIB)
            return()
        endif()
        #创建导入的全局库目标（IMPORTED GLOBAL），类型是根据 _LIB_TYPE 决定的（共享或静态
        ...
        #如果库包中存在 include 目录，则将其设置为库的 INTERFACE_INCLUDE_DIRECTORIES 属性，使得在编译时可以自动包含相应的头文件。
        ...
        # 将当前找到的库目标添加到 ${pkg_name}_LIBS 列表中
        list(APPEND ${pkg_name}_LIBS ${pkg_name}::${_LIB_NAME})
        message("found ${${_LIB_NAME}_LIB}")
        # 使用正则表达式提取库文件的路径（目录部分），并将其存储在 ${pkg_name}_LIBPATH 变量中供以后使用
...
function(isl_add_pkg pkg_name )
    #对传递给函数的参数进行分类和解析,结果存储在PKG_URL/PKG_CMAKE_PATH
    set(options )
    ...
    #通过包的名称和配置哈希值为该包设置一个唯一的存储目录路径。
    set(${pkg_name}_BASE_DIR ${CMAKE_BINARY_DIR}/../cmake-install)
    ...
    if(NOT PKG_DIR)
        ...
        if(IS_DIRECTORY ${PKG_DIR})
            ...
            # set(DECOMPRESS_DIR ${PROJECT_SOURCE_DIR}/external/${pkg_name})
